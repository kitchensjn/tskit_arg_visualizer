...
class D3ARG:
    ...
    num_samples : int
        The number of samples in the ARG (with (flags & 1) == 1)
    sample_order : list
    ...
    def from_json(cls, json):
        ...
        if json["plot_type"] == "full":
            samples = nodes.loc[bool(nodes["flags"] & tskit.NODE_IS_SAMPLE),["id", "fx"]]
    ...
    def _check_all_nodes_are_samples(self, nodes):
        ...
        for node in nodes:
            found = list(self.nodes.loc[self.nodes["id"] == int(node)]["flags"])
            if len(found) > 0:
                if len(found) == 1:
                    if found[0] != 1:
                        return False, node
    ...
    def _calculate_sample_order(self, order=None):
        ...
        for node in self.sample_order:
            ...
            if (found["flags"] & tskit.NODE_IS_SAMPLE) and found["id"] not in order:
    ...
    def _prepare_json(
        ...
        for index, node in nodes.iterrows():
            ...
            elif (node["flags"] & tskit.NODE_IS_SAMPLE) and (plot_type == "full"):
    ...
    def _map_node_ids_at_zoom(self, zoom):
        ...
        for edge in branch_lengths.itertuples():
            if counter >= zoom:
                break
            source_flags = self.nodes.loc[self.nodes["id"] == edge.source]["flags"].iloc[0]
            target_flags = self.nodes.loc[self.nodes["id"] == edge.target]["flags"].iloc[0]
